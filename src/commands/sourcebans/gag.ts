...
export class sourceGag {
  public run(client, message, args) {
    this.gag(client, message, args);
  }

  private async gag(client, message, args) {
  ...
  private async scriptLunch(page, nickname, steamid, length, reason) {
    ...
    await page.evaluate(
      (nickname, steamid, length, reason) => {
        xajax_AddBlock(nickname, 2, steamid, length, reason);
      },
      nickname,
      steamid,
      length,
