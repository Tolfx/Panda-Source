...
This code will ban an user. lol
...
export class sourceBan {
  public run(client, message, args) {
    this.ban(client, message, args);
  }

  private async ban(client, message, args) {
    ...
    const endURL = `https://bans.alisha.se/index.php?p=banlist&advSearch=${steamID}&advType=steamid`;

    const embed = new MessageEmbed().setColor("#5940D7")
      .setDescription(stripIndents`
    **Success**
    Banned user: ${nickname}
    SteamID: ${steamID}
    Length: ${length} min
    Reason: ${reason}
    ...
    const url = "https://bans.alisha.se/index.php?p=admin&c=bans";
    ...
    try {
      ...
      if (previousSession) {
        ...
        if (cookiesArr.length !== 0) {
          ...
          if (DoneResult === "true") {
            return log.normal("Everything was success in gag.ts");
  ...
  private async scriptLunch(page, nickname, steamid, length, reason) {
    ...
    await page.evaluate(
      (nickname, steamid, length, reason) => {
        xajax_AddBan(nickname, 0, steamid, "", length, "", "", reason, "");
      },
      nickname,
      steamid,
      length,
