...
private async commsBlock(message, args) {
        ...
        if (!args[0]) return message.channel.send(`You gonna search for air..? Use the right format: \`${config.General.prefix}listcomms [Username | SteamID]\``)
        ...
        try {
            ...
            let rawTextNoblock= await textContentNoBlock.jsonValue().then(async data => {
                ...
                if(data.trim() === `Total Blocks: 0`) {
                    message.channel.send(`Couldnt find any sort of communcation block on: \`${args[0]}\`, please try again.`);
                } else {
                    ...
                    let rawTextUnblocked = await textContentUnblocked.jsonValue().then(async text => {
                        ...
                        if(text.trim() === `CONSOLE`) {
                            ...
                            **Most recent communication block from \`${messageRespond[0]}\`**
                            **Unblocked**
...
private linksSourceban(args: any): Promise<string> {
        if(args.includes(`STEAM_`)) {
            return new Promise((resolve) => { resolve(`https://bans.panda-community.com/index.php?p=commslist&advSearch=${args}&advType=steamid`); });
        } else {
            return new Promise((resolve) => {resolve(`https://bans.panda-community.com/index.php?p=commslist&advSearch=${args}&advType=name`); });
        };
    };
};