...
export class sourceSilence {
  public run(client, message, args) {
    this.silence(client, message, args);
  }

  private async silence(client, message, args) {
    ...
    //If the user doesn't provide any sort of arguments here it wont work..
    if (!nickname)
      return message.channel.send(`You need to insert a nickname..`);
    ...
    const embed = new MessageEmbed().setColor("#5940D7")
      .setDescription(stripIndents`
    **Success**
    Silenced user: ${nickname}
    SteamID: ${steamID}
    Length: ${length} min
    Reason: ${reason}
    ...
    //This checks if it went well or not.
        ...
        if (cookiesArr.length !== 0) {
          ...
          if (!cookiesArr[0]) {
            ...
            if (cookiesArr[0].expires <= Date.now() / 1000) {
              ...
              log.normal("Adding the cookies for silence.ts");
              await page.setCookie(...cookiesArr);
            }
          }

          if (DoneResult === "true") {
            return log.normal("Everything was success in silence.ts");
  ...
  private async scriptLunch(page, nickname, steamid, length, reason) {
    ...
    await page.evaluate(
      (nickname, steamid, length, reason) => {
        xajax_AddBlock(nickname, 3, steamid, length, reason);
      },
      nickname,
      steamid,
      length,
