...
export default class listbans {
  ...
  private linksSourceban(args: any): Promise<string> {
    if (args.includes(`STEAM_`)) {
      return new Promise((resolve) => {
        resolve(
          `https://bans.panda-community.com/index.php?p=banlist&searchText=${args}&Submit=steamid`
        );
      });
    } else {
      return new Promise((resolve) => {
        resolve(
          `https://bans.panda-community.com/index.php?p=banlist&searchText=${args}&Submit=name`
        );
      });
    }
  }
